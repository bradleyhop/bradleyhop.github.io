...
function notOnlineDisplay(usr) {
    ...
    notOnline += `</div><div class='card-action notOnlineLink'>
        <a href='${ usr.page }'>Go To Channel</a>
        </div></div>`;
...
function onlineDisplay(usr) {
    ...
    if (usr.logoLink) {
        ...
        curStream += `<span class='card-title'>${ usr.display_name }</span>`;
    }
    if (usr.bio) {
    ...
    curStream += `</div><div class='card-action'>
        <a href='${ usr.online.linkToStream }'>Go To Stream</a>
        </div></div>`;
